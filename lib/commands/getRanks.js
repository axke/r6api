...
module.exports = function(platform, ids, opts) {
    ...
    const regionPromises = regions.map(region =>
        getAuthString().then(token => {
            ...
            return fetch(`${URLS[platform].RANK_URL}${querystring.stringify(qs)}`)(token);
        })
    );
    return Promise.all(regionPromises)
        .then(res =>
            res.reduce(function(map, regionResponse, i) {
                Object.keys(regionResponse.players).forEach(id => {
